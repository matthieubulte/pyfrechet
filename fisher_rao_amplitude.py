...
def d(x, y):
    ...
    return fdasrsf.utility_functions.elastic_distance(data_x, data_y, x.grid_points[0])[1]

def find_template_srsf(fd_q):
    srsf_data = fd_q.data_matrix[...,0]
    eval_points = fd_q.grid_points[0]
    ...
    return mu_idx, fd_q.copy(data_matrix=gammas_matrix, grid_points=eval_points)
...
def frechet_mean(y, w):
    ...
    template_idx, fd_y_warps = find_template_srsf(fd_q)
    gamma_mean = fisher_rao_warping_mean(fd_y_warps, w)
    gamma_inverse = invert_warping(gamma_mean)
    return y[template_idx].compose(gamma_inverse)
